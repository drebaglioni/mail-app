...
import { _setClientForTesting as _setAnthropicServiceClientForTesting } from "./anthropic-service";
...
export function _setClientForTesting(client: unknown): void {
  ...
  // Also wire the same mock into anthropic-service so callers that go through
  // llm-router (and on to anthropic-service directly) see the same mock.
  _setAnthropicServiceClientForTesting(client);
}
